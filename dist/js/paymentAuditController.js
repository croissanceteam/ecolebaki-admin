app.controller('PaymentsAuditCtrl', function ($scope, $http) {
  ...
  monthsArray[0] = {'month' : moment().format('MMMM YYYY')}; //declare un tableau avec un 1er item vide
  ...
  $scope.sendRequestTab = function () {
    ...
    if($scope.cbo_year != undefined && $scope.cbo_periodicity != undefined && (month != "" || day != "")){
      ...
      if(month != ""){
          ...
          var url = "getmonthpayupdates/" + startOfMonth + "/" + endOfMonth + "/" + $scope.cbo_year.year;
          console.log('URL :',url);
          ...
          $scope.reportTitle = "DETAILS DES MODIFICATIONS DES PAIEMENTS";
          $scope.labelTotal = "TOTAL JOURNALIER";
          $scope.period = day;
          console.log($scope.reportTitle);
          var url = 'getdaypayupdates/' + correctday + '/' + $scope.cbo_year.year;
          console.log('URL :',url);
          $http.get(url).then(function (response) {
            console.log('withError :',response.data.withError);
            console.log('Data :',response.data);
